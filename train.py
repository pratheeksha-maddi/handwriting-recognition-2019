#prg is to teach the alg to learn to recognise the alphabets. ,with the help of response, from the img.  
#!/usr/bin/python               .this prg is used to read a img with alphabets. in it. then detects the an area of 50 and puts a rectangle around it.
...
im = cv2.imread('../data/dataset1.png')
im3 = im.copy()
...
responses = []
keys = [i for i in range( 48, 58)] #makes the algoritm to read from 1-9

for cnt in contours:

    if cv2.contourArea(cnt) > 50:#countour array is greater than 50 is detected(i.e it detects a alphabets.(img) whose size is greater than 50 like area around the alphabets.) 
        [x, y, w, h] = cv2.boundingRect(cnt) #thedata i.e the height,width,x coord and ycord is stored

        if h > 28:

            cv2.rectangle(im, (x, y), (x + w, y + h), (128, 0, 111), 2)
            roi = thresh[y:y + h, x:x + w]
            roismall = cv2.resize(roi, (10, 10))#we are taking the img(the small alphabets..) and then storing it in roismall, we also reduce the size for storing 
